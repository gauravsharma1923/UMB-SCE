...
public class ExceptionControllerAdvice {
	...
	@ResponseBody
	public ResponseObj genericException(HttpServletRequest request,
			GenericException ex) {
		return new ResponseObj(null, ex.getStatus(), ex.getErrorDescription(),
				ex.getCode());
	}
	...
	public ResponseObj handleException(HttpServletRequest request, Exception ex) {
		...
		String errorMessage = ErrorCode.INTERNAL_SYSTEM_ERROR.getMessage(
				messageSource, null, LocaleContextHolder.getLocale());
	...
	@ResponseStatus(value = HttpStatus.OK)
	@ResponseBody
	public ResponseObj EmptyResultDataHandlr(HttpServletRequest request,
	...
	@ResponseStatus(value = HttpStatus.OK)
	@ResponseBody
	public ResponseObj handleException(HttpServletRequest request,
			ConstraintException ex) {
		Locale aLocale = Locale.forLanguageTag("en");
		String message = messageSource.getMessage("105",
				new Object[] { ex.getMessage() }, aLocale);
	...
	@ResponseBody
	public ResponseObj processValidationError(MethodArgumentNotValidException ex,
			Locale locale) {
		BindingResult result = ex.getBindingResult();
		List<FieldError> fieldErrors = result.getFieldErrors();
		ValidationErrorDTO errorDTO = processFieldErrors(fieldErrors, locale);
		return new ResponseObj(null, "Failed", errorDTO.getFieldError()
				.getMessage(), 120);
	}
	...
	private ValidationErrorDTO processFieldErrors(List<FieldError> fieldErrors,
		...
		for (FieldError fieldError : fieldErrors) {
			...
			String errorMessage = messageSource.getMessage(defaultMessage,
